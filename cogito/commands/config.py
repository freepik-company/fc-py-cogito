...
def version(ctx) -> None:
    ...
    try:
        ...
        if config_file.cogito and hasattr(config_file.cogito, "server") and hasattr(config_file.cogito.server, "version"):
        ...
        if latest_version > config_file.config_version:
            click.echo(f"Latest available config version: {latest_version} (upgrade available)")
        else:
...
@config.command()
@click.option('--backup/--no-backup', default=True, help='Create a backup before upgrading')
@click.pass_obj
def upgrade(ctx, backup) -> None:
    ...
    try:
        ...
        click.echo(f"Upgrading configuration from version {current_version} to {latest_version}")
